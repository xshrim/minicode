...
func ConvertByCalibre(file string) (resFile string, err error) {
	...
	*/
	file, _ = filepath.Abs(file)
	resFile = filepath.Dir(file) + "/" + strings.TrimSuffix(filepath.Base(file), filepath.Ext(file))
	...
	cmd := exec.Command("calibre ebook-convert", args...)
	/*
	...
	*/
	log.Println("calibre文档转换：%v %v", "calibre ebook-convert", strings.Join(args, " "))
...
func ExtractTextFromPDF(file string, from, to int) (content string) {
	...
	*/
	...
	if content == "" {
		os.Remove(textfile)
		textfile, _ = ConvertByCalibre(file)
		content = getTextFormTxtFile(textfile)
	}
	return
}
