...
launch_db() {
    ...
    $DOCKER_CMD run -d --name harbor-db -v ${PWD}//db:/home/postgres/database -v ${harbor_db_path}:/var/lib/postgresql/data ${harbor_db_image} "postgres"
}
...
dump_db() {
    $DOCKER_CMD exec harbor-db sh -c 'pg_dump -U postgres registry > /home/postgres/database/registry.back'
    $DOCKER_CMD exec harbor-db sh -c 'pg_dump -U postgres postgres > /home/postgres/database/postgres.back'
    $DOCKER_CMD exec harbor-db sh -c 'pg_dump -U postgres notarysigner > /home/postgres/database/notarysigner.back'
    $DOCKER_CMD exec harbor-db sh -c 'pg_dump -U postgres notaryserver > /home/postgres/database/notaryserver.back'
}

dump_database() {
    $DOCKER_CMD exec harbor-db sh -c 'rm -rf /home/postgres/db'
    $DOCKER_CMD exec harbor-db sh -c 'mkdir /home/postgres/db'
    $DOCKER_CMD exec harbor-db sh -c 'pg_dump -U postgres registry > /home/postgres/db/registry.bak'
    $DOCKER_CMD exec harbor-db sh -c 'pg_dump -U postgres postgres > /home/postgres/db/postgres.bak'
    $DOCKER_CMD exec harbor-db sh -c 'pg_dump -U postgres notarysigner > /home/postgres/db/notarysigner.bak'
    $DOCKER_CMD exec harbor-db sh -c 'pg_dump -U postgres notaryserver > /home/postgres/db/notaryserver.bak'
    $DOCKER_CMD cp harbor-db:/home/postgres/db harbor/
}
